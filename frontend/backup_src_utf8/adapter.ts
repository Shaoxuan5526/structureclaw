...
): 'beam' | 'column' | 'brace' | 'truss' | 'other' {
  if (type === 'truss') return 'truss'
  if (!start || !end) return type === 'beam' ? 'beam' : 'other'

  const dx = Math.abs(end.x - start.x)
  const dy = Math.abs(end.y - start.y)
  const dz = Math.abs(end.z - start.z)

  if (dz > dx && dz > dy) return 'column'
  if (type === 'beam' && dz < 1e-6) return 'beam'
  if (type === 'beam') return 'brace'
...
export function buildVisualizationSnapshot(params: {
  ...
  const elements: VisualizationElement[] = elementsInput
    ...
    .filter((item): item is Record<string, unknown> => Boolean(item && asStringId(item.id)))
    .map((element) => {
      const id = asStringId(element.id) as string
      const type = typeof element.type === 'string' ? element.type : 'beam'
      const nodeIds = pickElementNodeIds(element)

      const start = nodeIds[0] ? nodePositionById.get(nodeIds[0]) || null : null
      const end = nodeIds[1] ? nodePositionById.get(nodeIds[1]) || null : null
      ...
      return {
        ...
        displayType: resolveDisplayType(type, start, end),
        length: start && end ? computeDistance(start, end) : undefined,
      }
    })
