...
import { SteelFrameSummary } from './steel-frame-summary'
import {
  formatRestraints,
  getCaseMaxDisplacementMagnitude,
  isSteelFrameLike,
  resolveElementDisplayTypeLabel,
...
export function StructuralVisualizationModal({
  ...
  const showSteelFrameSummary = useMemo(() => isSteelFrameLike(snapshot), [snapshot])
  const sceneContainerRef = useRef<HTMLDivElement | null>(null)

  const handleCycleCase = useCallback((direction: 'prev' | 'next') => {
    if (!snapshot || snapshot.cases.length <= 1) return
    const currentIndex = snapshot.cases.findIndex((c) => c.id === activeCaseId)
    const count = snapshot.cases.length
    const nextIndex = direction === 'next'
      ? (currentIndex + 1) % count
      : (currentIndex - 1 + count) % count
    setActiveCaseId(snapshot.cases[nextIndex].id)
  ...
  const handleExportPng = useCallback(() => {
    ...
    try {
      ...
      a.href = url
      a.download = `structureclaw-${snapshot?.title || 'visualization'}-${activeCaseId || 'model'}.png`
      a.click()
    } catch {
      // canvas may be tainted in some environments — silently skip
    }
  }, [snapshot, activeCaseId])
  ...
  useEffect(() => {
    setDeformationScale((current) =>
      Math.min(deformationScaleRange.max, Math.max(deformationScaleRange.min, current))
    )
    ...
    <VisualizationModalShell
      ...
      aside={
        <div className="space-y-4">
          ...
          {snapshot && showSteelFrameSummary ? (
            <SteelFrameSummary
            ...
            <div className="rounded-2xl border border-border/70 bg-card/80 p-4 dark:border-white/10 dark:bg-slate-950/40">
              ...
              <div className="mt-3 space-y-2 text-sm text-muted-foreground">
                ...
                {selectedNode.restraints?.length ? (
                  <div>
                    {t('visualizationSupportRestraints')}: {formatRestraints(selectedNode.restraints)}
                  </div>
                ) : null}
                {!modelOnly && selectedNodeResults?.displacement ? (
                  <div>
                    {t('visualizationViewDeformed')}: {withUnit(
                      formatNumber(
                        Math.sqrt(
                ...
                {!modelOnly && selectedNodeResults?.reaction ? (
                  <div>
                    {t('visualizationViewReactions')}: {withUnit(
                      formatNumber(
                        Math.sqrt(
          ...
          {selectedElement ? (
            <div className="rounded-2xl border border-border/70 bg-card/80 p-4 dark:border-white/10 dark:bg-slate-950/40">
              <div className="text-xs uppercase tracking-[0.18em] text-muted-foreground">
                {t('visualizationSelectedElement')}
              </div>
              ...
              <div className="mt-3 space-y-2 text-sm text-muted-foreground">
                <div>
                  {t('visualizationElementDisplayType')}: {resolveElementDisplayTypeLabel(selectedElement, t)}
                </div>
                <div>{t('visualizationConnectedNodes')}: {selectedElementNodeIds.join(' - ')}</div>
                {selectedElement.material ? (
                  <div>{t('visualizationElementMaterial')}: {selectedElement.material}</div>
                ) : null}
                {selectedElement.section ? (
                  <div>{t('visualizationElementSection')}: {selectedElement.section}</div>
                ) : null}
                {typeof selectedElement.length === 'number' ? (
                  <div>
                    {t('visualizationElementLength')}: {withUnit(
                      formatNumber(selectedElement.length, locale),
                      snapshot?.lengthUnit || snapshot?.nodeLabelUnit
                    )}
                  </div>
                ) : null}
                {!modelOnly && typeof selectedElementResults?.axial === 'number' ? (
                  <div>{t('visualizationForceAxial')}: {withUnit(formatNumber(selectedElementResults.axial, locale), snapshot?.resultUnit)}</div>
                ) : null}
                {!modelOnly && typeof selectedElementResults?.shear === 'number' ? (
                  <div>{t('visualizationForceShear')}: {withUnit(formatNumber(selectedElementResults.shear, locale), snapshot?.resultUnit)}</div>
                ) : null}
                {!modelOnly && typeof selectedElementResults?.moment === 'number' ? (
                  <div>{t('visualizationForceMoment')}: {withUnit(formatNumber(selectedElementResults.moment, locale), snapshot?.momentUnit)}</div>
                ) : null}
          ...
          {selectedLoad ? (
            <div className="rounded-2xl border border-border/70 bg-card/80 p-4 dark:border-white/10 dark:bg-slate-950/40">
              <div className="text-xs uppercase tracking-[0.18em] text-muted-foreground">
                {t('visualizationSelectedLoad')}
              </div>
              <div className="mt-2 space-y-2 text-sm text-muted-foreground">
                <div>
                  {t('visualizationLoadType')}: {selectedLoad.kind === 'distributed'
                    ? t('visualizationLoadTypeDistributed')
                    : t('visualizationLoadTypeNodal')}
                </div>
                <div>
                  {t('visualizationLoadTarget')}: {selectedLoad.kind === 'distributed'
                    ? `${t('visualizationElementsList')} ${selectedLoad.elementId || '-'}`
                    : `${t('visualizationNodesList')} ${selectedLoad.nodeId}`}
                </div>
              </div>
              <div className="mt-3 space-y-2 text-sm text-muted-foreground">
                      ...
                      <button
                        ...
                        className={`rounded-full border px-3 py-1.5 text-xs transition ${
                          selectedNodeId === node.id
                            ? 'border-cyan-300/50 bg-cyan-300/14 text-foreground'
                            : 'border-border/70 bg-background/70 text-muted-foreground hover:text-foreground dark:border-white/10 dark:bg-white/5'
                        }`}
                        onClick={() => {
          ...
          {!selectedNode && !selectedElement && !selectedLoad ? (
            <div className="rounded-2xl border border-border/70 bg-card/80 p-4 text-sm leading-6 text-muted-foreground dark:border-white/10 dark:bg-slate-950/40">
              {snapshot
                ? `${t('visualizationSceneSelectionHelp')} ${t('visualizationSceneClickEmpty')}`
    ...
    >
      {snapshot && activeCase ? (
        <div className="flex h-full min-h-0 flex-col">
          ...
          />
          <div ref={sceneContainerRef} className="min-h-0 flex-1" data-testid="visualization-modal-scene">
            <StructuralScene
              activeCase={activeCase}
              deformationScale={deformationScale}
              forceMetric={forceMetric}
        ...
        <div className="flex h-full items-center justify-center px-6 py-10 text-center" data-testid="visualization-modal-placeholder">
          <div className="max-w-xl rounded-[28px] border border-dashed border-cyan-300/35 bg-cyan-300/8 p-8">
            <div className="text-sm uppercase tracking-[0.24em] text-cyan-700 dark:text-cyan-200">
