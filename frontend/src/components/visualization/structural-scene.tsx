...
import { Scene2DFallback } from './scene2d-fallback'
...
export function StructuralScene(props: StructuralSceneProps) {
  ...
  if (!webglAvailable) {
    // WebGL unavailable – show an interactive SVG 2D scene instead.
    return (
      <div
        className="relative h-full w-full bg-[radial-gradient(circle_at_top_left,rgba(34,211,238,0.06),transparent_24%),linear-gradient(180deg,rgba(148,163,184,0.06),transparent_30%)]"
        data-testid="visualization-scene-fallback"
      >
        <div className="pointer-events-none absolute left-3 top-3 z-10 rounded-lg border border-cyan-300/30 bg-background/80 px-2.5 py-1 text-[10px] uppercase tracking-widest text-cyan-600 dark:text-cyan-300">
          2D view
        </div>
        <Scene2DFallback
          ...
          deformationScale={props.deformationScale}
          forceMetric={props.forceMetric}
          selectedElementId={props.selectedElementId}
          selectedNodeId={props.selectedNodeId}
          showElementLabels={props.showElementLabels}
          showLoads={props.showLoads}
          showNodeLabels={props.showNodeLabels}
          showUndeformed={props.showUndeformed}
          view={props.view}
