    ...
    <div className="flex flex-wrap items-center gap-3 border-b border-border/70 px-4 py-3 dark:border-white/10">
      ...
      {onExportPng ? (
        <div className="ml-auto flex items-center">
          <button
            className="flex items-center gap-1.5 rounded-full border border-border/70 bg-background/70 px-3 py-2 text-sm text-muted-foreground transition hover:border-cyan-300/30 hover:text-foreground dark:border-white/10 dark:bg-white/5"
            onClick={onExportPng}
            title={t('visualizationExportPng')}
            type="button"
          >
            <Download className="h-3.5 w-3.5" />
            {t('visualizationExportPng')}
          </button>
        </div>
      ) : null}
    </div>
